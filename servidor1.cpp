/* Server code in C */
//Librerías necesarias, expecialmente socket que permite que utilicemos las palabras reservadas, etc.
#include <sys/types.h>
...
void movePiece(void)
{
  ...
  if (current_game->getCurrentTurn() == 1)
  {
    ...
    //Este comando imprimirá la información recibida

    bzero(buffer, 256);
    msg = move_from;
    //Con este comando el resultado ahora aparecerá en el cliente
    n = write(ConnectFD, msg.c_str(), msg.length());
    if (n < 0)
...
int main(void)
{
  ...
  //Se crea la variable SocketFD y se le asigna la familia PF_INET, el protocolo utilizado en la capa de transporte que es TCP por SOCKET_STREAM.
  //IPPROTO_TCP corresponde a los sockets de TCP creados para las familias de direcciones de IPv4 y IPv6
  SocketFD = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP);
  //Declaramos el tamaño del buffer

  //Este condicional nos ayuda a detectar un error y la ubicación de este.
  if (-1 == SocketFD)
  {
  ...
  //Aquí agregamos el tipo de familia (AF_INET)
  //La dirección que será cualquiera (INADDR_ANY)
  //Y el puerto 1101
  stSockAddr.sin_family = AF_INET;
  stSockAddr.sin_port = htons(1101);
  stSockAddr.sin_addr.s_addr = INADDR_ANY;

  //Se agrega otro condicional para volver a evaluar si hay algún error y notificarlo
  ...
  //Estado de escucha hasta que el cliente abra conexión y envíe información
  ...
  //Una vez el cliente envía la información, pasamos a este estado donde se crea un nuevo socket con la comunicación que se ha aceptado.
  //Este nuevo socket es ConnectFD
  for (;;)
  {
    ConnectFD = accept(SocketFD, NULL, NULL);

    //Aquí se verifica que no haya error
    if (0 > ConnectFD)
    {
    ...
    //Luego de haber aceptado la comunicación, se realiza una lectura de la información

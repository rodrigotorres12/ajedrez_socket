...
void movePiece(void)
{
  ...
  if (current_game->getCurrentTurn() == 0)
  {
    ...
    msg = move_from;
    //Aquí se ingresa el texto o números que queremos enviar al servidor
    n = write(SocketFD, msg.c_str(), msg.length());
    //Lee la información que le llega del server
...
int main(void)
{
  ...
  //Se agrega la información de la familia (AF_INET) y el puerto (1101)
  stSockAddr.sin_family = AF_INET;
  stSockAddr.sin_port = htons(1101);
  //Aquí sucede la conversión de texto a binario de las direcciones IPv4 y IPv6
  Res = inet_pton(AF_INET, "127.0.0.1", &stSockAddr.sin_addr);
  //Se evalúa si existe error
  if (0 > Res)
  {
  ...
  //Si no hay error entonces se procede con la conexión al socket, se establece la comunicación si no hay error.
